...
namespace SanteDB.Rest.OAuth
{
    internal static class ISymmetricCryptoServiceProviderExtensions
    {
        ...
        {
            return Base64UrlEncoder.Encode(provider.Encrypt(Encoding.UTF8.GetBytes(plainText)));
        }

        public static string DecryptString(this ISymmetricCryptographicProvider provider, string cipherText)
        {
            return Encoding.UTF8.GetString(provider.Decrypt(Base64UrlEncoder.DecodeBytes(cipherText)));
        }
