                ...
                if (address.Scheme == "https" && ep.CertificateBinding != null)
                {
                    try
                    {
                        ...
                        if (address.HostNameType == UriHostNameType.Dns)
                        {
                            ...
                            HttpSslUtil.GetCurrentPlatformCertificateBinder().UnbindCertificate(ipAddresses[0], address.Port, ep.CertificateBinding.Certificate.GetCertHash(), ep.CertificateBinding.StoreName, ep.CertificateBinding.StoreLocation);
                        }
                        else
                        {
                            HttpSslUtil.GetCurrentPlatformCertificateBinder().UnbindCertificate(IPAddress.Parse(address.Host), address.Port, ep.CertificateBinding.Certificate.GetCertHash(), ep.CertificateBinding.StoreName, ep.CertificateBinding.StoreLocation);
                        }
                    }
                    catch (Exception e)
                    {
