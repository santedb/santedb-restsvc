...
namespace SanteDB.Rest.Common
{
    ...
    {
        // Trace source
        private static Tracer m_tracer = Tracer.GetTracer(typeof(ObjectExpander));
        ...
        private static object DoPopulateObject(object scope, string property)
        {
            ...
            else if (propertyValue is Guid) // A key!
            {
                ...
                var rmi = typeof(ExtensionMethods).GetGenericMethod(nameof(SanteDB.Core.Model.ExtensionMethods.LoadProperty), new Type[] { backingFieldInfo.PropertyType }, new Type[] { scope.GetType(), typeof(String) });
        ...
        /// </summary>
        public static void ExcludeProperties(IdentifiedData returnValue, NameValueCollection qp, Stack<Guid> keyStack = null)
        {
            // Set the stack
            if (keyStack == null)
                keyStack = new Stack<Guid>();
